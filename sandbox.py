...
CAR_WINDOW_SOUND = pygame.mixer.Sound(
    os.path.join('stuff', 'sounds', 'car-window.ogg'))
RAIN_SOUND = pygame.mixer.Sound(os.path.join('stuff', 'sounds', 'rain.ogg'))
CAR_ENGINE_SOUND = pygame.mixer.Sound(
    os.path.join('stuff', 'sounds', 'driving-ambience.ogg'))
...
def load_image(path, scale, alpha=True, smooth=False):
    path = os.path.join('stuff', 'images', path)
...
class ParallaxScrollingImage(BaseObject):
    def __init__(self, x, y, image_paths, motion_factors, alpha=None, speed=100.0, scale=1.0):
        ...
        self.images = [ScrollingImage(
            x, y, path, scale=scale, alpha=alpha[i]) for i, path in enumerate(image_paths)]
        n = len(self.images)
        self.motion_factors = motion_factors if motion_factors else [
    ...
    def update(self, events: list[pygame.event.Event], dt):
        if self.timer.tick:
            self.particles.append(RainParticle(
                random.randint(0, int(W * 1.5)), -40, self.angle))
        self.particles = [i for i in self.particles if not i.destroyed]
        for i in self.particles:
            i.update(events, dt)

    def draw(self, surf: pygame.Surface):
        surf.fblits([(self.img, self.img.get_rect(center=i.pos))
                    for i in self.particles])
...
class Window(BaseObject):
    def __init__(self):
        ...
        self.s1 = self.s.copy()
        self.s2 = self.s.copy()
        # 24 FPS is the original FPS of recording of video
        self.timer = Timer(1 / 24)
        self.curr_frame = 0
    ...
    def pull_up(self, amt=1):
        self.rect = pygame.FRect(
            self.rect.x, self.rect.y - amt, self.rect.w, self.rect.h + amt)
    ...
    def draw(self, surf: pygame.Surface):
        ...
        self.s.blit(self.images[self.curr_frame], [0, 0],
...
class Game:
    def __init__(self):
        ...
        t = pygame.font.Font(os.path.join('stuff', 'fonts', '04B_30__.TTF'), 25).render(
            'Loading...', False, 'white')
